...
struct team {
  ...
  team() { tries = acc = t_acc = 0; }

  int calc_penalidade(int p) const { return t_acc + tries * p; }

  bool operator<(const team &vhs) const {
    if (acc == vhs.acc)
      return calc_penalidade(20) <
             vhs.calc_penalidade(
                 20);  // se a quantidade de problemas resolvidos for igual,
                       // retorna o menor tempo
    else
      return acc > vhs.acc;  // retorna quem acertou mais
  }
};
